...
package pe.gob.sunat.common.widgets.pshelf;
 ...
 * </p>
 * 
 * Instances of this class implement a selectable accordion metaphor, where each
 * shelf contains a client area.
 * <p>
 * The item children that may be added to instances of this class must be of
 * type <code>PShelfItem</code>. <code>Control</code> children are created on
 * the body composite of each items accessed via <code>PShelfItem#getBody</code>
 * .
 * </p>
 * <p>
 * <dl>
...
public class PShelf extends Canvas {
	...
	private static int checkStyle(int style) {
		int mask = SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT | SWT.BORDER
				| SWT.SIMPLE;
	 ...
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver or the
	 *                renderer has been disposed</li>
	 *                <li>ERROR_THREAD_INVALID_ACCESS - if not called from the
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		if (wHint == SWT.DEFAULT || hHint == SWT.DEFAULT) {
			if (openItem != null) {
				Point prefSize = openItem.getBody().computeSize(SWT.DEFAULT,
						SWT.DEFAULT);
	...
	private void onPaint(GC gc) {
		...
		for (Iterator iter = items.iterator(); iter.hasNext();) {
			...
			renderer.setBounds(0, y.intValue(), getClientArea().width,
					itemHeight);
	...
	private void animateOpen(PShelfItem previousItem, PShelfItem newItem,
			boolean openFromBottom) {
		...
		while (percentOfWork < 1) {
			...
			int yBottom = getClientArea().y
					+ getClientArea().height
					- (itemHeight * (items.size() - (items.indexOf(newItem) + 1)));

			int totalShrinkingGrowingArea = getClientArea().height
					- (itemHeight * items.size());

			int collapsingGrowingSpace = (int) (totalShrinkingGrowingArea * (openFromBottom ? 1 - percentOfWork
					: percentOfWork));
	...
	private void sizeClients() {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (clientHeight > 0) {
				item.getBodyParent().setVisible(true);
				item.getBodyParent().setBounds(0, y + itemHeight,
						getClientArea().width, clientHeight);
			} else {
	...
	void computeItemHeight() {
		...
		for (Iterator iter = items.iterator(); iter.hasNext();) {
			PShelfItem item = (PShelfItem) iter.next();
			itemHeight = Math.max(
					renderer.computeSize(gc, 0, SWT.DEFAULT, item).y,
					itemHeight);
		}
	...
	/**
	 * Returns the <code>PShelfItem</code> that is currently selected in the
	 * receiver.
	 * 
	...
	/**
	 * Returns an array of <code>PShelfItem</code>s which are the items in the
	 * receiver.
	 * <p>
	...
	/**
	 * Sets the speed of animation. This value indicates the relative
	 * growing/shrinking value for one step. Values between 0.0 and 1.0 are
	 * allowed.
	 * 
	 * @since 05.02.2016
	...
	/**
	 * Sets whether to redraw the items (previous and next item) when the open
	 * animation occours.
	 * 
	 * @since 05.02.2016
