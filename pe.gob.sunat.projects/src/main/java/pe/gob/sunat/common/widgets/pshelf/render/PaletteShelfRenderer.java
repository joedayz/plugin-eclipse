...
public class PaletteShelfRenderer extends AbstractRenderer {
	...
	public Point computeSize(GC gc, int wHint, int hHint, Object value) {
		...
		if (item.getImage() == null)
			return new Point(wHint, gc.getFontMetrics().getHeight()
					+ (2 * (margin + textMargin)));

		int h = Math.max(item.getImage().getBounds().height, gc
	...
	public void paint(GC gc, Object value) {
		...
		gc.fillRectangle(0, getBounds().y, getBounds().width - 1,
				getBounds().height - 1);

		gc.setForeground(shadeColor);

		gc.fillGradientRectangle(0, getBounds().y, 40, getBounds().height - 1,
				false);
		...
		gc.drawLine(getBounds().width - 1, getBounds().y,
				getBounds().width - 1, getBounds().y + getBounds().height - 1);
		...
		String text = getShortString(gc, item.getText(), getBounds().width - x
				- 4);
	...
	public void initialize(Control control) {
		...
		shadeColor = parent.getDisplay().getSystemColor(
				SWT.COLOR_WIDGET_NORMAL_SHADOW);
	}
