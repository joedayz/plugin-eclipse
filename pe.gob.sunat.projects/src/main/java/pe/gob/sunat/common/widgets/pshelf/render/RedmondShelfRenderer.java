 ...
 ******************************************************************************/
package pe.gob.sunat.common.widgets.pshelf.render;
...
public class RedmondShelfRenderer extends AbstractRenderer {
	...
	public Point computeSize(GC gc, int wHint, int hHint, Object value) {
		...
		if (item.getImage() == null) {
			...
			h = Math.max(item.getImage().getBounds().height, gc
					.getFontMetrics().getHeight() + (2 * textMargin));
		}
	...
	public void paint(GC gc, Object value) {
		...
		if ((parent.getStyle() & SWT.SIMPLE) != 0) {
			if (!isSelected()) {
				...
				gc.drawLine(0, getBounds().y, getBounds().width - 1,
						getBounds().y);
			}
		} else {
		...
		if (isSelected()) {
			gc.setFont(selectedFont);
			gc.setForeground(selectedForeground != null ? selectedForeground
					: fore);
		} else if (isHover()) {
			gc.setFont(font);
	...
	public void initialize(Control control) {
		...
		initialFont = new Font(parent.getDisplay(), fd.getName(),
				fd.getHeight(), SWT.BOLD);
		// parent.setFont(initialFont);

		Color baseColor = parent.getDisplay().getSystemColor(
				SWT.COLOR_TITLE_BACKGROUND_GRADIENT);
		Color color1 = createNewBlendedColor(baseColor, parent.getDisplay()
				.getSystemColor(SWT.COLOR_WHITE), 30);

		baseColor = createNewBlendedColor(
				parent.getDisplay().getSystemColor(
						SWT.COLOR_TITLE_BACKGROUND_GRADIENT), parent
						.getDisplay().getSystemColor(SWT.COLOR_WHITE), 80);
		...
		lineColor = createNewSaturatedColor(
				parent.getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION),
				.02f);

		baseColor = parent.getDisplay()
				.getSystemColor(SWT.COLOR_LIST_SELECTION);

		color1 = createNewBlendedColor(baseColor, parent.getDisplay()
		...
		if ((parent.getStyle() & SWT.SIMPLE) != 0)
			initialOpenFont = new Font(parent.getDisplay(), "Arial", 12,
					SWT.BOLD);
		else
			initialOpenFont = new Font(parent.getDisplay(),
					initialFont.getFontData());

		font = initialFont;
		selectedFont = initialOpenFont;

		Color inverseColor = parent.getDisplay().getSystemColor(
		...
		baseColor = createNewReverseColor(parent.getDisplay().getSystemColor(
				SWT.COLOR_TITLE_BACKGROUND));

		hoverGradient1 = createNewBlendedColor(baseColor, parent.getDisplay()
				.getSystemColor(SWT.COLOR_WHITE), 30);

		Color baseColor2 = createNewBlendedColor(baseColor, parent.getDisplay()
		...
		initialColors = new Color[] { gradient1, gradient2, selectedGradient1,
	...
	private static Color createNewBlendedColor(Color c1, Color c2, int ratio) {
		Color newColor = new Color(Display.getCurrent(), blend(c1.getRGB(),
				c2.getRGB(), ratio));
	...
	private static Color createNewReverseColor(Color c) {
		Color newColor = new Color(Display.getCurrent(), 255 - c.getRed(),
